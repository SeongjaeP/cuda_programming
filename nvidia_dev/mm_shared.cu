...
// Forward declaration of the matmul kernel
__global__ void MatMulKernel(const Matrix, const Matrix, Matrix);
// MatMul - Host code
// Matrix dimensions are assumed to be multiplie of BLOCK_SIZE
...
__global__ void MatMulKernel(Matrix A, Matrix B, Matrix C){
    ...
    // Mul each pair of sub-matrices together and accumulate the results
    for (int m = 0; m < (A.width / BLOCK_SIZE); m++) {
        // Get sub-matrix Asub of A
        Matrix Asub = GetSubMatrix(A, blockRow, m);
        // Get sub-matrix Bsub of B
        Matrix Bsub = GetSubMatrix(B, m, blockCol);
        // shared memory used to stroe Asub and Bsub respectively
